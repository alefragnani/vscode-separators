...
These instructions guide the Copilot Code Review agent when reviewing pull requests that add or update localization files for the Separators VS Code extension.
...
- **Example**: `automaticDetection.nls.es.md`, `selectSymbols.nls.pt-br.md`
...
- ✅ Nested properties preserved (e.g., `"separators.commands.toggleVisibility.title"`)
...
- ✅ Command links intact: `(command:separators.toggleVisibility)`
...
1. **Missing keys** - Incomplete localization
   ```
   Missing keys: ["separators.commands.navigateToNext.title", "separators.commands.navigateToPrevious.title"]
   ```
...
3. **Broken placeholders** - Runtime errors possible
   ```
   English: "Select Separator {0}"
   Localized: "Select Separator" // Missing {0}
   ```
...
1. **Extra keys** - May indicate stale translations
   ```
   Extra keys not in base file: ["separators.commands.oldCommand.title"]
   ```

2. **Untranslated values** - Values identical to English
   ```
   "separators.commands.toggleVisibility.title": "Toggle Visibility"  // Appears untranslated
   ```
...
- `separators.commands.toggleVisibility.title`
- `separators.configuration.newSetting.description`
...
- `separators.commands.oldFeature.title`
...
- `separators.walkthrough.title`: "Separators"

(Note: Some terms like "Separators" may be intentionally kept in English depending on localization guidelines)
```
...
1. **Product names** - May remain in English: "Separators", "VS Code"
2. **Technical terms** - Some terms may be transliterated rather than translated
3. **Command IDs** - Must never be translated: `command:separators.toggleVisibility`
4. **Setting keys** - Must never be translated: `separators.maxDepth`
